...
chartSeries(JSMR.JK)
```
Berdasarkan plot saham JSMR, terlihat adanya pola musiman yang mencerminkan fluktuasi harga yang terpengaruh oleh faktor eksternal, seperti kepadatan lalu lintas yang sering meningkat pada musim liburan atau hari raya. Pola ini menunjukkan bahwa permintaan dan aktivitas ekonomi, termasuk proyek infrastruktur yang dikelola oleh JSMR, dapat berfluktuasi secara signifikan tergantung pada waktu tertentu dalam setahun. Oleh karena itu, asumsi awal adalah menggunakan SARIMA
...
adf.test(df)
```
terlihat bahwa data tidak stasioner Nilai p-value untuk semua lag yang diuji tetap di atas tingkat signifikansi umum (α = 0.05), dengan p-value tertinggi mencapai 0.585 dan terendah 0.259. Oleh karena itu perlu differencing
...
Hasil analisis pemodelan ARIMA(2,0,0)(2,0,0)[12] menunjukkan bahwa model ini dapat menjadi kandidat yang baik untuk diterapkan dalam analisis data, mengingat kombinasi dua komponen autoregressive (AR) dan dua komponen musiman (SAR) yang dipilih secara otomatis.
...
auto_arima_model <- Arima(difs, order = c(2, 0, 0), seasonal = c(2, 0, 0))
auto_arima_model
```
...
n_ahead <- 10  # Tentukan jumlah langkah prediksi yang diinginkan
...
```
Prediksi menunjukkan bahwa baik volatilitas (deviasi standar) maupun varians akan meningkat selama 10 langkah ke depan, dengan deviasi standar diperkirakan mencapai 96.26 pada T+10 dan varians mencapai 9265.08. Hal ini mencerminkan ekspektasi adanya fluktuasi harga yang lebih besar dan peningkatan risiko di masa mendatang.
